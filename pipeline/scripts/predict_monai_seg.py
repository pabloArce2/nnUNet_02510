...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run MONAI liver segmentation inference")
    ...
    p.add_argument("--save-probability", action="store_true", help="Also save liver probability maps")
    return p.parse_args()
...
def main() -> None:
    ...
    for batch in tqdm(loader, desc="Predict", leave=False):
        ...
        image_path = Path(batch["image_meta_dict"]["filename_or_obj"][0])
        ...
        pred_np = mask[0].detach().cpu().numpy().astype(np.uint8)
        nib.save(nib.Nifti1Image(pred_np, ref.affine, ref.header), str(pred_dir / f"{case_id}.nii.gz"))

        if args.save_probability:
            prob_np = probs[0].detach().cpu().numpy().astype(np.float32)
            nib.save(nib.Nifti1Image(prob_np, ref.affine, ref.header), str(prob_dir / f"{case_id}.nii.gz"))
